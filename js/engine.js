...
async function inicializarMundo() {
  ...
  if (saved && saved.cronologia && saved._dataVersion === DATA_VERSION) {
    ...
    try {
      const regras = await fetch(`data/regras_globais.json?v=${DATA_VERSION}`, { cache: 'no-cache' }).then(r => r.json());
    ...
    if (!temEN) {
      try {
        const [clubesEN, jogadoresEN] = await Promise.all([
          fetch(`data/en/clubes.json?v=${DATA_VERSION}`, { cache: 'no-cache' }).then(r => r.json()),
          fetch(`data/en/jogadores.json?v=${DATA_VERSION}`, { cache: 'no-cache' }).then(r => r.json()),
        ]);
  ...
  const [regras, clubesBR, jogadoresBR, eventosBR, clubesEN, jogadoresEN, eventosEN] = await Promise.all([
    fetch(`data/regras_globais.json${v}`, { cache: 'no-cache' }).then(r => r.json()),
    fetch(`data/br/clubes.json${v}`, { cache: 'no-cache' }).then(r => r.json()),
    fetch(`data/br/jogadores.json${v}`, { cache: 'no-cache' }).then(r => r.json()),
    fetch(`data/br/eventos.json${v}`, { cache: 'no-cache' }).then(r => r.json()),
    fetch(`data/en/clubes.json${v}`, { cache: 'no-cache' }).then(r => r.json()),
    fetch(`data/en/jogadores.json${v}`, { cache: 'no-cache' }).then(r => r.json()),
    fetch(`data/en/eventos.json${v}`, { cache: 'no-cache' }).then(r => r.json()),
  ]);
